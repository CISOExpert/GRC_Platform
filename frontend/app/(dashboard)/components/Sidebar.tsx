...
const Sidebar = () => {
    ...
    return (
        <nav
            className={`bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 min-h-screen flex flex-col transition-all duration-300 ${
                isCollapsed ? 'w-16' : 'w-64'
            }`}
        >
            {/* Logo/Brand with Collapse Toggle */}
            <div className={`border-b border-gray-200 dark:border-gray-800 ${isCollapsed ? 'p-3' : 'p-4'}`}>
                <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
                    {!isCollapsed && (
                        <div className="flex-1 min-w-0">
                            <h2 className="text-lg font-bold text-indigo-600 dark:text-indigo-400 truncate">GRC Platform</h2>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">SCF-Centric Compliance</p>
                        </div>
                    )}
                    <button
            ...
            <div className={`border-t border-gray-200 dark:border-gray-800 ${isCollapsed ? 'p-2' : 'p-4'}`}>
                <div className={`flex items-center text-xs text-gray-500 dark:text-gray-400 ${isCollapsed ? 'justify-center' : 'gap-2'}`}>
                    <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                    </svg>
                    {!isCollapsed && <span>v2024 SCF</span>}
                </div>
            </div>
        </nav>
    )
